...
TitleScene::TitleScene()
{
	...
	class TestObj : public Component
	{
		...
		void Render()
		{
			...
			DrawStringToHandle(L"あsu\nshiいう\nsus\nhi↑え↑お┃か", Colors::White, font, Matrix3::CreateTranslation(obj_pos) * world);
			...
			if (InputManager::GetInstance().key->GetButton('W') || InputManager::GetInstance().key->GetButton(VK_UP))
				DrawStringToHandle(L"↑", Colors::White, font, Matrix3::CreateTranslation(Vector2{ 15, 35 }) * world);
			if (InputManager::GetInstance().key->GetButton('S') || InputManager::GetInstance().key->GetButton(VK_DOWN))
				DrawStringToHandle(L"↓", Colors::White, font, Matrix3::CreateTranslation(Vector2{ 15, 45 }) * world);
			if (InputManager::GetInstance().key->GetButton('A') || InputManager::GetInstance().key->GetButton(VK_LEFT))
				DrawStringToHandle(L"←", Colors::White, font, Matrix3::CreateTranslation(Vector2{ 10, 40 }) * world);
			if (InputManager::GetInstance().key->GetButton('D') || InputManager::GetInstance().key->GetButton(VK_RIGHT))
				DrawStringToHandle(L"→", Colors::White, font, Matrix3::CreateTranslation(Vector2{ 20, 40 }) * world);
