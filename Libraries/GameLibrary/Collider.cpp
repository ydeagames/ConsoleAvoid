...
static CollisionResult CollisionCircleSegment(const Circle& _circle, const Vector2& _circle_vel, const Vector2& _p1, const Vector2& _p2) {
	...
	if (cross >= 0) {
		translate = Vector2::right.Rotate((_p2 - _p1).Angle() - MathUtils::PI / 2) * (_circle.size + 1);
	}
	else {
		translate = Vector2::right.Rotate((_p2 - _p1).Angle() + MathUtils::PI / 2) * (_circle.size + 1);
	}
...
void BoxCollider::Apply(const CollisionResult & result) const
{
	...
	if (sinf(rotate_angle) < 0) {
		transform->rotation = MathUtils::Loop(-rotate_angle - rotate, MathUtils::PI * 2);
	}
	auto rigidbody = gameObject()->GetComponent<Rigidbody>();
	if (rigidbody)
...
CollisionResult BoxCollider::Collide(const CircleCollider& other) const
{
	...
	if (vertex_distance[0] < vertex_distance[1]) {
		...
		if (result.hit) {
			t_a = result.time;
			_time = result.time;
			_ref_normal = _rect.angle + MathUtils::PI;
			is_collision = true;
		}
	}
	//回転矩形の右側に近い場合
	...
	if (vertex_distance[1] < vertex_distance[2]) {
		...
		if (result.hit) {
			...
			if (!is_collision || t_b < t_a) {
				_time = result.time;
				_ref_normal = _rect.angle + MathUtils::PI * 3 / 2;
				is_collision = true;
			}
		}
	}
	//回転矩形の下側に近い場合
		...
		if (result.hit) {
			...
			if (!is_collision || t_b < t_a) {
				_time = result.time;
				_ref_normal = _rect.angle + MathUtils::PI / 2;
				is_collision = true;
			}
		}
	}
	...
	if (is_collision && MathUtils::FloatEquals(t_a, t_b)) {
		_ref_normal = _rect.angle + MathUtils::PI * 5 / 4 + MathUtils::PI / 2 * min_distance_index;
	}
