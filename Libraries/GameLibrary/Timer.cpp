...
void Timer::Tick()
{
	tick = std::chrono::duration_cast<std::chrono::microseconds>(Time::GetInstance().delta_first).count();
}
...
{
	if (tick < 0)
		return std::chrono::duration_cast<std::chrono::microseconds>(Time::GetInstance().delta_first).count();
	return tick;
}
