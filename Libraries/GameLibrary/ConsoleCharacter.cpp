...
namespace ConsoleCharacter
{
	...
	*/
	int GetCharacterWidth(wchar_t ucs)
	{
		...
		if (Interval::bisearch(ucs, combining,
			sizeof(combining) / sizeof(Interval) - 1))
			return 0;
	...
	*/
	int GetCharacterWidthCJK(wchar_t ucs)
	{
		...
		* characters, generated by "uniset +WIDTH-A -cat=Me -cat=Mn -cat=Cf c" */
		...
		/* binary search in table of non-spacing characters */
		if (Interval::bisearch(ucs, ambiguous,
			sizeof(ambiguous) / sizeof(Interval) - 1))
			return 2;

		return GetCharacterWidth(ucs);
	}
	...
	int GetStringWidthCJK(const wchar_t* pwcs, size_t n)
	{
		...
		for (; *pwcs && n-- > 0; pwcs++)
			if ((w = GetCharacterWidthCJK(*pwcs)) < 0)
				return -1;
			else
				width += w;
