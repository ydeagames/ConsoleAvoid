...
namespace Easings
{
	...
	const Easing INELASTIC = { [](float t, float b, float c, float d) {
		...
		if (a < fabsf(c)) {
			...
			s = p / (2 * PI)*std::asinf(c / a);
		return -(a*std::powf(2, 10 * (t -= 1))*std::sinf((t*d - s)*(2 * PI) / p)) + b;
	} };
	...
	const Easing OUTELASTIC = { [](float t, float b, float c, float d) {
		...
		if (a < fabsf(c)) {
			...
			s = p / (2 * PI)*std::asinf(c / a);
		return a * std::powf(2, -10 * t)*std::sinf((t*d - s)*(2 * PI) / p) + c + b;
	} };
	...
	const Easing INOUTELASTIC = { [](float t, float b, float c, float d) {
		...
		if (a < fabsf(c)) {
			...
			s = p / (2 * PI)*std::asinf(c / a);
		if (t < 1)
			return -.5f*(a*std::powf(2, 10 * (t -= 1))*std::sinf((t*d - s)*(2 * PI) / p)) + b;
		return a * std::powf(2, -10 * (t -= 1))*std::sinf((t*d - s)*(2 * PI) / p)*.5f + c + b;
	} };
