...
namespace CXLib
{
	CXImage::PixelImage::PixelImage(const std::string & path, ColorCode transparent)
	{
	...
	CXImage::CXImage(const std::shared_ptr<PixelImage>& image, int x, int y, int w, int h)
		: image(image)
		, x(x)
		, y(y)
		, w(w)
	...
	CXImage::CXImage(const std::string & path, ColorCode transparent)
	...
	void CXImage::DrawGraph(const Matrix3& world) const
	{
	...
	CXImage CXImage::DerivationGraph(Vector2 start, Vector2 size) const
	{
		int x1 = x + start.X();
		int y1 = y + start.Y();
	...
	CXImage CXImage::DerivationGraphUV(Vector2 startUV, Vector2 sizeUV) const
	{
		return DerivationGraph(startUV * Vector2{ w, h }, sizeUV * Vector2{ w, h });
	}

	CXImage LoadGraph(const std::string& path, ColorCode transparent)
	{
		return{ path, transparent };
	}

	void DrawGraph(const CXImage& image, const Matrix3& world)
	{
